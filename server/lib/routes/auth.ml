...
module Query = struct
  ...
  let validate_user_credentials email password pool =
    ...
    in
    let* result =
      Caqti_lwt.Pool.use (fun db -> query db ~email ~password) pool
    in
    match result with
...
let routes pool =
  [ Dream.get "/me" (fun request ->
      ...
      | Some user_id ->
        ...
        let* user_opt = Query.find_user_by_id user_id pool in
        begin
          match user_opt with
          | Some user ->
            let user_json = Query.yojson_of_t user in
  ...
  ; Dream.post "/login" (fun request ->
      ...
      let user = body |> Yojson.Safe.from_string |> t_of_yojson in
      let* user_opt =
        Query.validate_user_credentials user.email user.password pool
      in
      match user_opt with
      | Some user ->
